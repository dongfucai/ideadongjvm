...
public enum  OpcodeRout{
    ...
    RETURN(Constants.RETURN){
        @Override
        public void invoke(Env env, StackFrame frame, byte[] operands) throws Exception{
            frame.setReturn(null,"void"); // 设置返回值和返回类型
        }
    },
    /**
     * 获取对象的静态字段值
    ...
    GETSTATIC(Constants.GETSTATIC){
        ...
        public void invoke(Env env, StackFrame frame, byte[] oprands) throws Exception{
            ...
            // ConstantPool.CONSTANT_Fieldref_info 常量池中的字段信息
            // CONSTANT_Fieldref_info就是对一个字段的符号引用， 这个符号引用包括两部分，
...
//    },
    ...
    LDC(Constants.LDC){
        ...
        public void invoke(Env env, StackFrame frame, byte[] operands) throws Exception{
            ...
            ConstantPool.CPInfo info = frame.getConstantPool().get(index);
    ...
    DADD(Constants.DADD){
      @Override
      public void invoke(Env env, StackFrame frame, byte[] operands){
    ...
    NEW(Constants.NEW){
        ...
        public void invoke(Env env, StackFrame frame, byte[] operands) throws Exception{
            ...
            JvmClass clazz = env.getVm().getClass(info.getName());// 没有的话 先加载这类
            frame.getOperandStack().push(clazz.newInstance(env)); // 创建实力
    ...
    PUTFIELD(Constants.PUTFIELD){
        ...
        public void invoke(Env env, StackFrame frame, byte[] operands) throws Exception {
            ...
            Object objectref = frame.getOperandStack().pop();// 指实例对象
